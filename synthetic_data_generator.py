...
class MedicalDataGenerator:
    ...
    def generate_normal_record(self, report_id: str, candidate_id: str, report_date: str) -> Dict:
        ...
        for category, options in self.categorical_options.items():
            if category in ['ABO_BloodGrp', 'Rh_Factor']:
                ...
                record[category] = 'Absent' if category == 'Urine_RBC' else 'Negative'
                if random.random() < 0.1:  # 10% chance of abnormal
                    record[category] = 'Present' if category == 'Urine_RBC' else 'Positive'
    ...
    def generate_altered_record(self, report_id: str, candidate_id: str, report_date: str) -> Dict:
        ...
        if alteration_type == 'glucose_inconsistency':
            ...
            # Change blood type for same candidate (impossible)
            different_types = [t for t in self.categorical_options['ABO_BloodGrp'] 
                             if t != record['ABO_BloodGrp']]
            record['ABO_BloodGrp'] = random.choice(different_types)
    ...
    def generate_dataset(self, n_records: int = 1000, altered_ratio: float = 0.5) -> pd.DataFrame:
        ...
        column_order = [
            ...
            'Sodium', 'Potassium', 'ALP', 'Cholesterol', 'LDL', 'HDL', 'Triglycerides',
...
synthetic_data = generator.generate_dataset(n_records=1000, altered_ratio=0.5)
...
# Save to CSV
synthetic_data.to_csv('synthetic_medical_dataset.csv', index=False)
print(f"\nDataset saved as 'synthetic_medical_dataset.csv'")
